...
class ComparisonSettings:
    ...
    def validate(self) -> List[str]:
        ...
        if not isinstance(self.max_differences_display, int) or self.max_differences_display <= 0:
            errors.append("Максимальное количество отображаемых различий должно быть положительным числом")
...
class AppConfig:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.max_recent_files, int) or self.max_recent_files <= 0:
            errors.append("Максимальное количество недавних файлов должно быть положительным числом")
