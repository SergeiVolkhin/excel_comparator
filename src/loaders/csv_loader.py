...
class CSVFileLoader(IFileLoader):
    ...
    def load(self, file_path: Path, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            raise FileLoadError(str(file_path), f"Ошибка кодировки: {e}")
        except pd.errors.EmptyDataError:
            raise FileLoadError(str(file_path), "Файл не содержит данных")
        except pd.errors.ParserError as e:
            raise FileLoadError(str(file_path), f"Ошибка парсинга CSV: {e}")
    ...
    def _detect_encoding(self, file_path: Path) -> str:
        ...
        try:
            ...
            if confidence < 0.7:
                self.logger.warning(f"Низкая уверенность в кодировке ({confidence}), используем UTF-8")
                encoding = 'utf-8'
    ...
    def _prepare_load_params(self, separator: str, encoding: str, **kwargs) -> Dict[str, Any]:
        ...
        allowed_params = [
            'header', 'index_col', 'skiprows', 'skipfooter', 'nrows',
            'na_values', 'keep_default_na', 'dtype', 'converters',
            ...
            'date_parser', 'dayfirst', 'cache_dates'
        ]
