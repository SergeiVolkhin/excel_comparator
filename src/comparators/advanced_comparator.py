...
class AdvancedComparator(IComparator):
    ...
    def compare(self, df1: pd.DataFrame, df2: pd.DataFrame, **options) -> ComparisonResult:
        ...
        key_columns = options.get('key_columns', None)
        
        self.logger.info(f"Начало сравнения DataFrames {df1.shape} vs {df2.shape}")
        
        # Выравниваем столбцы и строки
        aligned_df1, aligned_df2, column_stats, row_stats = self.align_dataframes(df1, df2, key_columns)
    ...
    def align_dataframes(self, df1: pd.DataFrame, df2: pd.DataFrame, 
                      key_columns: list = None) -> Tuple[pd.DataFrame, pd.DataFrame, Dict, Dict]:
        """
        ...
        if key_columns:
            ...
            if missing_keys:
                raise ComparisonError(f"Ключевые столбцы {missing_keys} отсутствуют в общих столбцах")
    ...
    def _create_differences_mask(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        ...
        differences_mask = pd.DataFrame(
            df1_filled.values != df2_filled.values,
            columns=df1.columns,
            index=df1.index
        )
