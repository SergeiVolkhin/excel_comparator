...
from .widgets import ProgressDialog, SettingsDialog, FileSelector
...
class MainWindow:
    ...
    def _setup_menu(self) -> None:
        ...
        file_menu.add_command(label="Новое сравнение", command=self._new_comparison, accelerator="Ctrl+N")
    ...
    def _setup_widgets(self) -> None:
        ...
        ttk.Label(main_frame, text="Сохранить результат как:").grid(row=3, column=0, sticky=tk.W, pady=(20, 5))
        ...
        # Тип сравнения
        ttk.Label(options_frame, text="Тип сравнения:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
    ...
    def _run_comparison(self, file1_path: str, file2_path: str, 
                      output_path: str, progress_reporter: ProgressReporter) -> None:
        ...
        if not recent_files:
            ...
            for file_path in recent_files[:10]:  # Показываем только последние 10
                ...
                self.recent_menu.add_command(
                    label=file_name,
                    command=lambda f=file_path: self._load_recent_file(f)
                )
        ...
        if Path(file_path).exists():
            ...
            if not self.file1_selector.get_path():
                ...
                choice = messagebox.askyesno(
                    "Замена файла",
                    "Оба файла уже выбраны. Заменить первый файл?"
                )
                if choice:
                    self.file1_selector.set_path(file_path)
                else:
