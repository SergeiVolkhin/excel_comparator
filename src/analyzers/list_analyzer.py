...
from ..core.interfaces import IDifferenceAnalyzer, DifferenceDetail
...
class ListDifferenceAnalyzer(IDifferenceAnalyzer):
    ...
    def _analyze_lists(self, old_list: List[str], new_list: List[str]) -> Dict[str, Any]:
        ...
        # Проверка изменения порядка (только если наборы элементов похожи)
        order_changed = False
        if not added and not removed and len(frequency_changes) == 0:
            # Проверяем порядок только для случаев, когда содержимое одинаковое
