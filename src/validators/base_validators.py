...
class ShapeValidationRule(IValidationRule):
    ...
    def validate(self, df1: pd.DataFrame, df2: pd.DataFrame) -> List[str]:
        ...
        if df1.shape != df2.shape:
            errors.append(
                f"Размеры файлов не совпадают: {df1.shape} vs {df2.shape}"
            )
...
class SizeValidationRule(IValidationRule):
    ...
    def validate(self, df1: pd.DataFrame, df2: pd.DataFrame) -> List[str]:
        ...
        if df1.shape[0] > self.max_rows:
            errors.append(f"Первый файл слишком большой: {df1.shape[0]} строк (максимум {self.max_rows})")
        
        if df2.shape[0] > self.max_rows:
            errors.append(f"Второй файл слишком большой: {df2.shape[0]} строк (максимум {self.max_rows})")
        
        # Проверяем количество столбцов
        if df1.shape[1] > self.max_columns:
            errors.append(f"Первый файл содержит слишком много столбцов: {df1.shape[1]} (максимум {self.max_columns})")
        
        if df2.shape[1] > self.max_columns:
            errors.append(f"Второй файл содержит слишком много столбцов: {df2.shape[1]} (максимум {self.max_columns})")
